...
const TableCreation = function (act, index) {
    ...
    var items = `<tr  class="table-light child-row">
                        <td  width ="1%"><div class="pl-3"><input id="${act.userTasksId}" value="${index + 1}-${act.employee.name}(${act.startDate} - ${act.endDate})" class="form-check-input" type="checkbox" /></div> </td>                     
...
$(document).ready(function () {
    dataTable = $('#dt_tasks').DataTable({
        ...
        'paging': false,
        "ajax": {
            //"url": "/Manager/InternalPlan/GetAllTasks?month=September 2023&teamLeadId=2",
            "url": "/TeamLead/InternalPlan/GetAllTasks",

            "type": "POST",
            "dataType": "json",
            //"data": {
            //    month: $("#dt_month").val(),
            //    teamLeadId: $("#cmb_team_lead_filter").val()
            //},
            //"traditional": true
        },
        ...
        columns: [
            ...
            },
            {
                "data": "",
                "render": function (draw, type, row, data) {
                    return `<input  class="form-check-input checkbox" type="checkbox" />`;
                },
                "width": "1%",
                'orderable': false,
                "className": "text-center",
