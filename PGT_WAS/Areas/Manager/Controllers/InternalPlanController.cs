...
namespace PGT_WAS.Areas.Manager.Controllers
{
    ...
    public class InternalPlanController : Controller
    {
        ...
        public async Task<IActionResult> Edit(PrimaryuserTasks vm)
        {
            try
            {
                //int[] taskActivityIds = vm.TaskActivityIds.Select(str => Convert.ToInt32(str.Split("-")[0])).ToArray();
                //int[] assignedTaskids = vm.TaskActivityIds.Select(str => Convert.ToInt32(str.Split("-")[1])).ToArray();
        ...
        public async Task<IActionResult> GetTeamEmployees(int? teamlead)
        {
            try
            {
                if (!COM.IsValidCount(teamlead))
                {
                    return NotFound();
                }

                var employees = (from emp in await _unitWork.Employee.GetAll(includeProp: $"{nameof(Employee.ReportingTo)},{nameof(Employee.Department)},{nameof(Employee.Designation)},{nameof(Employee.Location)}")
                                 where emp.RepotingPersonId == teamlead
                                 select emp);
